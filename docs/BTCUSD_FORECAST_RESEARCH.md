...
| BTC-R019 | High | Feature consumption | The engine can build and report an exogenous matrix for a method that never consumes it; classical Holt/Theta/naive/Fourier, analog, Monte Carlo, and TimesFM paths accept the call but ignore `exog_used`. A successful feature run can therefore be mislabeled as a feature model. | Restrict feature experiments to source-verified consumers and require effective-feature diagnostics plus a raw-control comparison. | Declare multivariate and future-covariate capabilities per method, reject incompatible requests, and report whether each prepared matrix was consumed. | Open |
