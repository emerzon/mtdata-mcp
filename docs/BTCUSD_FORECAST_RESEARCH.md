...
| BTC-R032 | High | Anchor scheduling | Public `forecast_backtest_run` exposes backward rolling `steps`/`spacing`, although the engine has an internal explicit-anchor path. Horizon and feed gaps can change the origin phase, so the exact preregistered UTC grids cannot yet be guaranteed through the CLI. | Keep A1 and all later model calls paused. Use only one-origin contract smokes until a public exact-anchor request is implemented and the harness verifies returned anchors byte-for-byte against a prehashed list. | Expose bounded explicit ISO-UTC anchors in the public request/CLI, echo the resolved list, and fail on missing, shifted, duplicate, or out-of-window origins. | Open; blocks A1 |
...
### Iteration 4 audit and integrity pause
...
resumed, and supplies no selection evidence. Research remained paused while the
harness was hardened and committed. A fresh source-pinned run will repeat the
audit with at least 800 candle rows and explicit RSI/ROC/NATR catalog records.
