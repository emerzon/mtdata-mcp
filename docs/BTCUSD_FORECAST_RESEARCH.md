...
| BTC-R007 | Medium | Model isolation | Trainable rolling tests and tuners can write to the default model store; not every command has an ephemeral-cache flag. | The harness now gives every research command attempt a fresh model store and jobs database, inventories and hashes its files, and keeps materialize/shadow in a separate shared lifecycle store. | Add an explicit ephemeral/store-root option consistently to training and evaluation commands. | Fixed in harness; CLI option open |
...
| BTC-R011 | Medium | Experiment tracking | General CLI results are stdout JSON/NDJSON; there is no cross-command study ledger, immutable holdout lock, or resume mechanism. | Persist manifests, immutable attempt-level raw responses, normalized results, failures, and decisions in the experiment driver. | Add a first-class read-only research-run registry. | Mitigated in harness; product support open |
| BTC-R012 | Low | Scale | Backtests are capped at 200 anchors, so a single command cannot cover an arbitrarily long H1 grid. | Use bounded explicit anchor lists and split longer grids at preregistered role boundaries. | Add streaming/windowed evaluation for grids above 200 anchors. | Explicit lists fixed; streaming open |
...
| BTC-R027 | High | Interval replay contract | Native conformal output contains one cutoff's forecast bounds and aggregate calibration diagnostics, while full backtest output has point paths but no per-anchor interval bounds. Full-detail backtests now expose realized target timestamps, which solves only the point-outcome side of the join. | Keep interval approval disabled. A future harness replay must preregister conformal cutoffs, preserve each raw envelope, and bind every bound to the exact backtest target timestamp. | Expose a first-class timestamped per-anchor interval/outcome ledger with interval bounds. | Point timestamps fixed; interval ledger open |
...
| BTC-R031 | Critical | Artifact integrity | The experiment harness originally recorded mutable raw paths without SHA-256 digests, trusted completed files on resume, and overwrote failed output on retry. Attempt databases could also leave uninventoried WAL/SHM/quarantine files. A changed response could therefore be normalized as if it were original MT5 evidence. | The harness now writes immutable `attempt-N` envelopes, hashes and revalidates every attempt, inventories model and complete SQLite file families, uses fresh attempt stores, pins clean source plus Python/package identity, and stops on the first failed command. The pre-fix Iteration 4 audit bundle is quarantined and will not be resumed. | Make content-addressed response artifacts and resumable research-run provenance a first-class mtdata facility. | Fixed in harness |
| BTC-R032 | High | Anchor scheduling | Public `forecast_backtest_run` previously exposed only backward rolling `steps`/`spacing`; horizon and feed gaps could change the origin phase. | The CLI now accepts 1–200 exact UTC anchors. The engine preflights the complete list before fitting, fails rather than shifting or dropping an origin, echoes requested/resolved lists, and emits realized timestamps. The harness hashes one canonical grid into the stage and every command, then verifies it byte-for-byte. | Keep explicit grids and their resolution policy as a first-class public evaluation contract. | Fixed in this change |
| BTC-R033 | High | Research scoring | Native backtest rankings do not implement Iteration 4's terminal-return balanced accuracy, matched raw/no-op checks, fixed-H next-open economics, ISO-week cluster bootstrap, multiplicity diagnostics, or transition gates. Parsing results ad hoc would make selection outcome-dependent. | Keep A1 parsing paused until a tested, outcome-free offline scorer implements the frozen protocol and verifies raw hashes and anchor identity. | Add reusable terminal-return and cluster-aware evaluation primitives, or a versioned research scorer contract. | Open; blocks A1 parsing |
| BTC-R034 | High | Lifecycle provenance | The harness's latest-materialization pointer is not yet fully bound by contained-path resolution and a manifest/artifact/raw hash chain. A modified pointer could select the wrong model or escape the study directory before shadow use. | Do not materialize or start shadow forecasting until the pointer chain is hardened and tested. | Bind the pointer, manifest record, candidate/pipeline/as-of fields, model ID, artifact digest, and source raw digest with contained paths. | Open; blocks materialize/shadow |
| BTC-R035 | High | Smoke verification | The first hardened smoke verifier recomputed the screen `plan_digest` in manifest map order. JSON persistence sorts map keys, while the harness hashes preregistered variant order, so a valid LightGBM feature/raw pair was rejected before evidence parsing and the immutable failed artifact ended the run. | The failed run is retained and no later model call was made. The verifier now reconstructs the frozen six-variant order before checking the digest, with a regression fixture that serializes a differently ordered command map. | Store command order explicitly beside future plan digests or define them over an order-independent canonical map. | Fixed in verifier |
| BTC-R036 | Medium | CLI regression drift | The full CLI suite still asserted obsolete `features lag/rolling` examples after the public help had moved to the supported indicator/covariate contract. This blocked a clean anchor-contract verification run and risked reintroducing invalid guidance. | Replace the stale assertion and retain both a valid key/value covariate example and the causal JSON example in generated help. | Generate CLI examples and tests from one feature-schema fixture. | Fixed in this change |
| BTC-R037 | High | Timestamp precision | The first public explicit-anchor draft normalized request values to UTC seconds while the internal engine echoed minute precision. Exact list/hash verification would reject every otherwise valid grid. | Standardize requested, resolved, detail-anchor, entry, analysis, and realized timestamps on second-precision UTC for explicit mode; cover the cross-layer contract in tests. | Centralize canonical bar-open formatting for request, engine, CLI, and research artifacts. | Fixed in this change |
| BTC-R038 | High | Error propagation | The internal exact-anchor preflight returned a structured error code and per-origin issues, but the normal public execution wrapper initially converted it to a message-only exception. CLI/MCP callers would lose the evidence needed to distinguish a missing origin from insufficient history or overlap. | Preserve this fail-closed preflight payload through the public wrapper while retaining legacy exception behavior for unrelated forecast errors; test wrapper and use-case field survival. | Provide a general typed structured-error transport instead of one-off wrapper exceptions. | Fixed in this change |
...
### Iteration 4 audit and contract smoke

The first Iteration 4 audit completed all ten read-only MT5/catalog commands,
but no feature or raw-control model command was started. The 500-candle probe
found historical spread on only 237 rows (47.4%), so missing spread cannot be
treated as zero. Return stationarity passed all three requested diagnostics;
the seasonality probe found no strong cycle; the outlier audit flagged 52 of
800 observations across its requested fields; and the current regime response
classified the latest window as ranging with a bullish window bias. These are
workflow diagnostics, not evidence of forecast skill.

That run then exposed BTC-R031: its manifest had mutable raw paths but no raw
digests. The bundle at
`backtests/btcusd_forecast/iter4_smoke_dac74f5d/` is quarantined, will never be
resumed, and supplies no selection evidence.

The next source-pinned attempt, `iter4_smoke_a524d681`, completed its audit and
the LightGBM raw/momentum pair. Its verifier stopped before parsing model
evidence because of BTC-R035's plan-order mismatch. The immutable failed
validation artifact ends that run; no later command was made.

The clean `iter4_smoke_37c79980` rerun completed all 13 audit commands and all
six preregistered H12 contract calls. Its aggregate verifier passed source,
runtime, invocation, store-inventory, hash-chain, parameter, feature-consumption,
matched-actual, and non-no-op checks. Maximum feature-versus-raw path deltas
ranged from `0.0009512427124235348` to `0.0013261758835681477`. These values
show that the features reached prediction and changed outputs; six calls at one
anchor say nothing about predictive skill.

The 800-row H1 audit had complete cadence and no gaps, but only 285 rows
(35.62%) carried historical spread. BTC-D004 therefore remains active: missing
spread is never converted to zero, and fixed disclosed cost scenarios remain
the economic evidence boundary. Formal A1 model calls were then paused for
BTC-R032. Exact public anchors and harness verification are now implemented;
BTC-R033's frozen offline scorer remains the next blocker before A1 outcomes
are parsed or used.
